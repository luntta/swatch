...
describe("toLinearRGB: sRGB gamma removal", () => {
	it("maps black to 0 and white to 1", () => {
		expect(tincture("#000000").toLinearRGB()).toEqual({ r: 0, g: 0, b: 0 });
		const w = tincture("#ffffff").toLinearRGB();
	...
	it("maps mid-gray 128/255 to ~0.2159", () => {
		...
		const lin = tincture("#808080").toLinearRGB();
...
describe("toXYZ: D65 reference white", () => {
	it("white sRGB → D65 XYZ (≈ 0.9505, 1.0000, 1.0888)", () => {
		const xyz = tincture("#ffffff").toXYZ();
	...
	it("black sRGB → (0, 0, 0)", () => {
		const xyz = tincture("#000000").toXYZ();
	...
	it("pure red sRGB → (0.4124, 0.2126, 0.0193)", () => {
		const xyz = tincture("#ff0000").toXYZ();
...
describe("toLab: CIE Lab D65", () => {
	it("white → L=100, a=0, b=0", () => {
		const lab = tincture("#ffffff").toLab();
	...
	it("black → L=0, a=0, b=0", () => {
		const lab = tincture("#000000").toLab();
	...
	it("pure red → L≈53.24, a≈80.09, b≈67.20", () => {
		...
		const lab = tincture("#ff0000").toLab();
	...
	it("mid-gray #808080 has near-zero chroma", () => {
		const lab = tincture("#808080").toLab();
...
describe("toLch: LCh(ab)", () => {
	it("pure red → L≈53.24, C≈104.55, h≈40°", () => {
		const lch = tincture("#ff0000").toLch();
	...
	it("white has L=100 and C=0", () => {
		const lch = tincture("#ffffff").toLch();
...
describe("toOklab (Ottosson 2020)", () => {
	it("white → (1, 0, 0)", () => {
		const ok = tincture("#ffffff").toOklab();
	...
	it("black → (0, 0, 0)", () => {
		const ok = tincture("#000000").toOklab();
	...
	it("pure red ≈ (0.628, 0.225, 0.126)", () => {
		...
		const ok = tincture("#ff0000").toOklab();
	...
	it("pure green ≈ (0.866, -0.234, 0.179)", () => {
		const ok = tincture("#00ff00").toOklab();
	...
	it("pure blue ≈ (0.452, -0.032, -0.312)", () => {
		const ok = tincture("#0000ff").toOklab();
...
describe("toOklch", () => {
	it("red has positive chroma and hue ~29°", () => {
		...
		const lch = tincture("#ff0000").toOklch();
